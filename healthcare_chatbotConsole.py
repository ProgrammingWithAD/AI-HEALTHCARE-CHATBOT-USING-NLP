...
def execute_bot():
    ...
    print(Fore.CYAN + "Please reply with yes/Yes or no/No for the following symptoms")
    ...
    def tree_to_code(tree, feature_names):
        ...
        def recurse(node, depth):
            ...
            if tree_.feature[node] != _tree.TREE_UNDEFINED:
                ...
                print(Fore.BLUE + f"\nSymptoms given: {list(symptoms_given)}")
                confidence_level = (1.0*len(symptoms_present))/len(symptoms_given)
                print(Fore.MAGENTA + f"\nConfidence level is {confidence_level}")
                print(Fore.CYAN + "\nThe model suggests:")
                row = doctors[doctors['disease'] == present_disease[0]]
                print(Fore.WHITE + f"Consult {row['name'].values}")
                print(Fore.WHITE + f"Visit {row['link'].values}")
